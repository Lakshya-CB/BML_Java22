...
//		LRS
	...
	public void lvlOrder2() {
		...
		Queue<Node> nxt_Q = new LinkedList<Node>();
		...
		while (!curr_Q.isEmpty()) {
			...
			if (curr.right != null) {
				nxt_Q.add(curr.right);
			}
			if (curr_Q.isEmpty()) {
				curr_Q = nxt_Q;
				nxt_Q = new LinkedList<Node>();
				System.out.println();
			}
		}
		System.out.println();
	...
	private Node createlvl(int[] arr) {
		...
		while (!Q.isEmpty()) {
			Node curr = Q.poll();
			if (idx < arr.length && arr[idx] != -1) { // Left child
				Node nn = new Node(arr[idx]);
				curr.left = nn;
				Q.add(nn);
			}
			idx++;
			if (idx < arr.length && arr[idx] != -1) {// Right child
				Node nn = new Node(arr[idx]);
				curr.right = nn;
				Q.add(nn);
			}
...
//		Pre L =>ps+1 to ps+Lsize
//		In L  =>is to found -1;

		nn.left = createPreIn(pre, ps + 1, ps + Lsize, in, is, found - 1);
//		Pre P => ps+Lsize+1 to pe
//		In R => found +1 to ie;
		nn.right = createPreIn(pre, ps + Lsize + 1, pe, in, found + 1, ie);
		return nn;
	}
...
//			min in RIght SubTree
			return nn.data >= L_max && nn.data < R_min;
